...
namespace CompassInterviewTest
{
    public class OpacityCalculatorConstants
    {
        ...
        public const double ChosenFrequencyToExahertz = 1.0E18;
        ...
        private static readonly Func<MaterialType, double, double> LinearAttenuationFunction(MaterialType materialType, double v)
        {
            ...
            if (chosenVolts > ElectronVoltsB && chosenVolts < ElectronVoltsC) bottomVolt = ElectronVoltsB;
            // Question, what do I do if it's greater than C? Not sure if that's possible given the real world values being used though

            //Linear interpolation says lower bound to upper bound in parameter order
            //So while I know the lower volt number, all the material coefficients are actually in reverse from that
